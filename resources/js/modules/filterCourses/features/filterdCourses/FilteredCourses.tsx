...
const FilteredCourses = ({ data, openSidebar }: { data: any[]; openSidebar: () => void }) => {
    return (
        <div className="flex h-fit w-full lg:w-auto flex-col gap-y-8 ">
            <button
                onClick={openSidebar}
            ...
            {data.length === 0 && 1 ? (
                ...
                <div className="gap grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap">
                    {data.map((course: any) => (
                        <Card
                            title={course.grade}
