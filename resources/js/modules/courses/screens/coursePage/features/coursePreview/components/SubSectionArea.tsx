...
const SubSectionArea = ({
  ...
  if (loading || !subSection) {
    return <Skeleton className="xl:h-[520px]  w-full rounded-[30px]" />;
  }

  return (
    <div className="xl:h-[520px] bg-red-700 main-rounded w-full">
      {subSection.type === 'video' && subSection.url ? (
        <video
          className="main-rounded size-full object-cover"
          src={subSection.url}
          controls
        />
        ...
        <div className="main-rounded flex h-full flex-col items-center justify-center gap-y-4 bg-white p-6 text-center">
          ...
          <button
            onClick={() =>
              alert(`ابدأ الامتحان: ${subSection.examId || 'غير معروف'}`)
            }
        ...
        <div className="main-rounded flex h-full items-center justify-center text-gray-500">
          لا يوجد محتوى لعرضه
        </div>
      )}
    </div>
