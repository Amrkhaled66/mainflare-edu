...
export const useFilterOptions = () => {
    ...
    const handlePriceRangeChange = useCallback((min: number, max: number) => {
        setOptions((prevOptions) => {
            if (prevOptions.price_range[0] === min && prevOptions.price_range[1] === max) {
                return prevOptions;
            }
            return {
                ...prevOptions,
    ...
    return useMemo(
        ...
        [options, appliedOptions, handleOptionChange, handlePriceRangeChange, resetFilters, applyFilters, updateURLParams],
    );
};
