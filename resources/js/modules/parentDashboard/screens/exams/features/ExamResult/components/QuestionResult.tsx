...
const QuestionResult = ({ answers }: { answers: any[] }) => {
    return (
        <div className="dashBoard-subContainer h-fit flex-1 space-y-6">
            <p className="section-header">التنقل بين الاسئلة</p>
            <div className="space-y-3">
                <div className="flex flex-wrap gap-x-2 gap-y-3 border-b border-stroke pb-3 lg:gap-x-3">
                    {answers.map((answer: any, index: number) => {
                        const isCorrect = answer.isCorrect;
                        return (
                            <p
                                key={index}
                                className={`flex-center size-8 gap-3 rounded-full border text-center text-white sm:size-10 lg:size-14 ${isCorrect ? 'bg-greenColor' : 'bg-wrong'} `}
                            >
                                {index + 1}
                            </p>
                        );
